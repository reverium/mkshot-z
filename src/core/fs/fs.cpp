...
static void initReadOps(PHYSFS_File *handle, SDL_IOStream &io) {
...
struct FSPrivate {
    ...
    // TODO: see if OneShot relies on such functionality
    //
...
void FS::addPath(const char *path, const char *mountpoint, bool reload) {
    ...
    if (!state) {
        // If it didn't work, try mounting via a wrapped SDL_IOStream
...
static PHYSFS_EnumerateCallbackResult cacheEnumCB(void *d, const char *origdir,
    ...
    if (shState && shState->rtData().rqTerm)
        throw Exception(Exception::MKShotError, "Game close requested. Aborting path cache enumeration.");
